...
$user = null;
$stmt = $conn->prepare("SELECT id_utilizator, username, email, nume_complet, data_creare FROM utilizatori WHERE id_utilizator = ? LIMIT 1");
...
if (!$user) {
  // dacă sesiunea e “coruptă” sau userul nu mai există
  header("Location: logout.php");
  exit;
}

// Luăm programările utilizatorului (dacă există tabela programari)
$programari = [];
$sqlProg = "
  SELECT pr.id_programare, pr.data_ora, pr.observatii,
         p.nume_parc,
